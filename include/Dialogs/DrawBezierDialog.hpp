...
#include<iostream>
using namespace std;
class DrawBezierDialog:public QDialog{
private:
Q_OBJECT
...
gp_Pnt nextPoint;
gp_Pnt startPoint;
Handle(CustomAIS_Shape) bezierShape;
...
DrawBezierDialog(QWidget* parent=nullptr):QDialog(parent){
    setWindowTitle(tr("Draw Bezier Dialog"));
    ...
    if(okButton){
     connect(okButton,&QPushButton::clicked,this,&DrawBezierDialog::OnHandleOk);      
    }
    
    connect(relativeAngleEdit.get(),&DoubleEdit::GetValue,this,&DrawBezierDialog::OnGetValueFromRelativeAngleEdit);
    connect(lengthEdit.get(),&DoubleEdit::GetValue,this,&DrawBezierDialog::OnGetValueFromLengthEdit);
    

    connect(dialogButtons.get(),&QDialogButtonBox::rejected,this,&DrawBezierDialog::OnHandleCancel);
    vlayout->addWidget(dialogButtons.get());
    setLayout(vlayout.get());
    setSizeGripEnabled(true);
    connect(xDirectButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
    connect(yDirectButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
   connect(zDirectButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
   connect(inXButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
   connect(inYButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
     connect(inZButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
    connect(negInXButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
    connect(negInYButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
    
    connect(negInZButton.get(),&QRadioButton::toggled,this,&DrawBezierDialog::IsClicked);
    return;
}
...
std::vector<gp_Pnt> pnts;
 ...
 gp_Pnt prevPoint=pnts.at(pnts.size()-1);
  ...
  if(pnts.size()>Geom_BezierCurve::MaxDegree()+1){
    return;
  }
  NCollection_Array1<gp_Pnt> arrayOfPoints(pnts.front(),0,pnts.size()-1);
  Handle(Geom_BezierCurve) beziercurve=new Geom_BezierCurve(arrayOfPoints);
  if(!beziercurve){
    return;
  }
  ...
  if(!bezierShape){
    bezierShape=new CustomAIS_Shape(edgemaker.Edge());
    bezierShape->SetZLayer(Graphic3d_ZLayerId_Topmost);
    bezierShape->SetColor(Quantity_NOC_AQUAMARINE2);
    context->Display(bezierShape,true);
    return;
  }
  
  bezierShape->SetShape(edgemaker.Edge());
  CheckDisplayStatus(bezierShape,context->DisplayStatus(bezierShape));
  return;
}
...
void OnHandleCancel(){
    if(context->IsDisplayed(bezierShape)){
        context->Erase(bezierShape,true);
    }
