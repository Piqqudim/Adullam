...
QMenu *DataFlowGraphicsScene::createSceneMenu(QPointF const scenePos)
{
    ...
    for (auto const &cat : registry->categories()) {
        auto item = new QTreeWidgetItem(treeView);  //it is same as treeview->addTreeWidgetItem(item);
        item->setText(0, cat);
    ...
    connect(treeView,
            ...
            [this, modelMenu, scenePos](QTreeWidgetItem *item, int) {
                ...
                if(item->text(0)==tr("Indexer")){
                 OnSendString(item->text(0));
                 modelMenu->close();
                   return;
                 }
                if(item->text(0)==tr("Part Index")){
                    OnSendString(item->text(0));
                     modelMenu->close();
                   return;
                 }
