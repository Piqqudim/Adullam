...
{
    std::shared_ptr<NodeDelegateModel> model = _registry->create(nodeType);
...
void DataFlowGraphModel::loadNode(QJsonObject const &nodeJson)
{
    ...
    std::shared_ptr<NodeDelegateModel> model = _registry->create(delegateModelName);
    ...
    if (model) {
        ...
        _models[restoredNodeId]->load(internalDataJson);//what a useful code
    } else {
